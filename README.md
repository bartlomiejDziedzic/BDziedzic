...
## Wstęp
Portfolio prezentuje moje umiejętności zdobyte na kierunku Inżynieria Informatyki w Akademii Tarnowskiej oraz z innych zródeł. Strona ma prosty, responsywny układ, by wygodnie oglądać ją na komputerze i urządzeniach mobilnych. Zawiera: stronę główną, sekcję umiejętności, listę projektów, szczegóły projektu oraz galerię obrazów.
...
5. **gallery.html**
    - Galeria obrazów użytych w projektach. Skrypt `js/gallery.js` generuje układ w CSS Grid z efektem powiększenia i nazwą przy hoverze.
...
- **HTML**: semantyczne znaczniki (`<header>`, `<nav>`, `<main>`, `<footer>`, `<figure>`, `<figcaption>`), meta tagi (UTF-8, viewport, Open Graph).
- **CSS**:
    - Flexbox i Grid do układów.
    - Media queries: dla szerokości ≥768px i ≤1023px oraz ≤767px, zmienia sposób formowania strony, ukrywać tabelę na małych ekranach, wyświetlać karty.
    - Efekty hover dla przycisków i obrazóww galerii.

- **JavaScript**:
    - `nav.js`: buduje i podświetla menu.
    - `projects.js`:
        - Fetch JSON z API, przekształca dane na tabelę i karty.
        - Funkcja `sortArray` sortuje po nazwie lub dacie wykonania.
...
- **JSON API**: zdalne źródło danych projektów (tutaj: myjson.online), łatwa aktualizacja portfolio bez zmian HTML.
